...
export async function ListSynthesisJobs(userID: string, offset?: string) {
  const params = [userID]
  if (offset) {
    params.push(offset)
  }
  const q = await pool.query(`SELECT * FROM synthesis_jobs WHERE user_id = $1 ${offset ? 'AND id > $2' : ''}`, params)
